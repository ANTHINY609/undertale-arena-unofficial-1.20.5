...
###Prepare Item
item replace entity 0-0-0-0-1 container.0 with carrot_on_a_stick[minecraft:custom_data={soul:1b,canned:0b,nonboss:0b,loot_soul:1b,can_fist:1b},minecarft:attribute_modifiers={modifiers:[],show_in_tooltip:false},minecraft:item_name='{"text":"","color":"white","italic":false,"bold":true,"extra":[{"translate":"itm.bosssoul","with":[{"translate":"tem.0"}]}]}'minecraft:lore:[]] 1

###Modify Model
scoreboard players operation #cmd tid = @s tid
execute store result entity 0-0-0-0-1 Item.components."minecraft:custom_data".soultype int 1 run scoreboard players get @s tid
